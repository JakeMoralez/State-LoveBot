...
def register_chat_admin(bot: Bot, api: API, action_logger: ActionLogger) -> None:
    ...
    @bot.on.message(text=dual("invite"))
    @requires_level(AccessLevel.SUPERVISOR)
    async def invite_usage(
        message: Message,
        server_id: int = 0,
        access_level: int = 0,
    ) -> None:
        ...
        if not reply_id:
            await message.answer(
                "❌ /invite [@user]\n"
                "Или ответом: /invite\n"
                "Нужен VK_USER_TOKEN с правом «Сообщения»."
            )
            return
        if reply_id == (message.from_id or 0):
            await message.answer("❌ Нельзя пригласить самого себя.")
            return
        ok, err = await admin.invite_member(message.peer_id, reply_id)
        link = await names.link_user(reply_id, server_id)
        if ok:
            await message.answer(f"✅ {link} добавлен(а) в беседу.", disable_mentions=1)
            ...
            await message.answer(f"❌ Не удалось добавить {link}.\n{err}", disable_mentions=1)
            await action_logger.log_user(
                "invite",
                message.from_id,
                f"id{reply_id}",
                f"Ошибка: {(err or 'нет прав')[:80]}",
                source_peer_id=message.peer_id,
            )

    @bot.on.message(text=dual_with_args("invite", "<args>"))
    @requires_level(AccessLevel.SUPERVISOR)
    async def invite_user(
        ...
        target_id = await _resolve_target(message, args, server_id)
        if not target_id:
            await message.answer("❌ Пользователь не найден.\n/invite [@user]")
            return
        if target_id == (message.from_id or 0):
            await message.answer("❌ Нельзя пригласить самого себя.")
            return

        ok, err = await admin.invite_member(message.peer_id, target_id)
